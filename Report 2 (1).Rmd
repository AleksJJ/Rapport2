...
### Reliability between lactate threshold at 4 and 2 mmol
...
library(kableExtra)
library(knitr)
lactate_threshold <- read_excel("lactate_threshold.xlsx", col_types = c("numeric", "text", "numeric", "numeric", "numeric", "numeric", "numeric", "numeric", "numeric", "numeric", "numeric", "numeric"))
...
dat %>% 
  select(cv4, cv2) %>%
  kable(format="html",  
        col.names = c("CV 4 mmol (%)", "CV 2 mmol (%)"),
        caption = "Table 1. Typical percentage error at lactate thresholds at 4 and 2 mmol.") %>%
        add_footnote(label= "CV = Typical percentage error",
                     notation = "none") %>%
  kable_styling(latex_options = "striped") 
```
The typical percentage error (CV) of the lactate threshold at 4 mmol was 12.17% and 15.39% at 2 mmol. The CV between the two lactate thresholds does not differ in a big extend, with approximately 3% between the values. This means that the precision of the measures at 2 and 4 mmol is accurate. 